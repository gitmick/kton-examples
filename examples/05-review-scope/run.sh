...
# --add opens the scope AND files the seed in one step; the scope id is in the output.
SCOPE="$(nekton seed drug-review --sign "$PWD/.work/keys/chair.key" --by "CN=Chair" --add | grep -oE 'sha256:[0-9a-f]+' | head -1)"
echo "  scope id = $SCOPE"
...
C2="$(mkclaim urn:doc:results  "results approved"  "$C1")"
echo "  link1 = $C1"
echo "  link2 = $C2"
...
nekton head "$SCOPE"
# A claim with a dangling prev does NOT extend the chain: it is persisted (its prev might resolve from
# another peer later - incomplete, not invalid), but it never joins the scope, so it is never the head.
